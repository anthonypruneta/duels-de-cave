...
7. **Donjons / simulerMatch — pas de double préparation**: `simulerMatch(char1, char2)` appelle **toujours** `preparerCombattant()` sur ses deux arguments. Lui passer des données **brutes** (ex. `character`, `createBossCombatant()`, `buildFloorEnemy()`). Ne **jamais** passer un objet déjà préparé (ex. `player` issu de `prepareForCombat(character)`), sinon forêt/arme/passif sont appliqués deux fois. Pour l’affichage, garder `player`/`boss` préparés dans le state ; pour la simulation, appeler `simulerMatch(character, bossBrut)`.
