    ...
    <div className={`${wrapperClassName} w-full max-w-[1800px] mx-auto overflow-x-hidden`}>
      ...
      <div className="hidden lg:flex gap-4 items-stretch justify-center text-sm">
        ...
        <div className="flex-1 min-w-[400px] flex flex-col">
          <div className="rounded-lg border border-stone-600 bg-stone-900/80 px-3 py-2 text-[11px] text-stone-400 mb-3">
          ...
          <div className="bg-stone-900/90 border border-stone-600 rounded-lg px-3 py-2 mb-3">
            ...
            <div className="grid gap-2 mt-2">
              {(run.lineup || []).map((boss, i) => {
                ...
                return (
                  <div key={i} className={`rounded-lg px-2 py-1.5 border transition ${
                    rowBossHighlight ? 'border-red-400 bg-red-950/40' : isActive ? 'border-amber-600/80 bg-stone-900/80' : 'border-stone-700 bg-stone-900/40 opacity-70'
                  }`}>
                    <div className="flex justify-between text-[11px] text-stone-400 mb-1 gap-2 items-center">
                      <span className="flex items-center gap-2 min-w-0 text-stone-300 font-medium">
                        {sprite ? <img src={sprite} alt="" className="w-9 h-9 object-contain flex-shrink-0" style={{ imageRendering: 'pixelated' }} /> : null}
                        <span className={`truncate ${COOP_RED_BOSS_NAME_COLORS[boss.nom] ?? 'text-stone-300'}`}>{boss.nom}</span>
                      </span>
                      <span className="flex-shrink-0">{cur} / {maxH}</span>
                    </div>
                    ...
                    {moveUi && (
                      <p className="text-[10px] text-amber-200/90 mt-1.5 leading-tight line-clamp-2" title={`${moveUi.name} — ${moveUi.description}${moveUi.cooldownLabel ? ` (${moveUi.cooldownLabel})` : ''}`}>
                        <span className="text-stone-500">Sort :</span> {moveUi.name}
                        {moveUi.cooldownLabel ? <span className="text-stone-500 font-normal"> · {moveUi.cooldownLabel}</span> : null}
                      </p>
                    )}
                  </div>
                );
