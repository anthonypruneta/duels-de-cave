...
const ExtensionDungeon = () => {
  ...
  const formatLogMessage = (text) => {
    ...
    text.split(nameRegex).forEach((part) => {
      ...
      if (part === bName) {
        parts.push(<span key={`name-${key++}`} className="font-bold text-violet-400">{part}</span>);
        return;
      }
      ...
      while ((match = numRegex.exec(part)) !== null) {
        if (match.index > lastIndex) {
          pushWithCritHighlight(parts, part.slice(lastIndex, match.index));
        }
      ...
      if (lastIndex < part.length) {
        pushWithCritHighlight(parts, part.slice(lastIndex));
      }
    });
