          ...
          <section className="rounded-xl border-2 border-red-700 bg-red-950 px-4 py-4 shadow-xl">
            ...
            {isPermissionError && (
              <div className="mt-3 rounded-lg border border-red-800/80 bg-black/30 px-3 py-2 text-xs text-red-100/90 leading-relaxed">
                ...
                <p className="mt-1">
                  Déploie la dernière version de <code className="rounded bg-black/40 px-1">firestore.rules</code>{' '}
                  sur ton projet Firebase (chemin{' '}
                  <code className="rounded bg-black/40 px-1">
                    pvpDuelStatsByUser/&#123;userId&#125;/pvpDuelCharStats/&#123;charId&#125;
                  </code>
                  , <code className="rounded bg-black/40 px-1">allow read: request.auth != null</code>).
                </p>
              </div>
            )}
            {error.toLowerCase().includes('index') && (
              <p className="mt-3 text-xs text-red-200/90">
                ...
                <code className="rounded bg-black/40 px-1">firestore.indexes.json</code> (champ{' '}
                <code className="rounded bg-black/40 px-1">wins</code> DESC sur{' '}
                <code className="rounded bg-black/40 px-1">pvpDuelCharStats</code>).
              </p>
            )}
          </section>
        )}
