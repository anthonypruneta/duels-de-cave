      ...
      <div className="min-h-screen p-6">
        ...
        <div className="max-w-[1800px] mx-auto pt-20">
          ...
          <CombatLayout
            ...
            logTitle="🔥 Forge des Légendes"
            logTitleClass="text-base font-bold text-orange-300 text-center"
            ...
            renderLog={() => combatLog.length === 0 ? (
              ...
              <>
                {combatLog.map((log, idx) => {
                  ...
                  if (!isP1 && !isP2) {
                    if (log.includes('🏆') || log.includes('🔨')) return <div key={idx} className="flex justify-center my-3"><div className="bg-orange-600/90 text-white px-4 py-2 rounded-lg font-bold text-sm shadow-lg border border-orange-400">{cleanLog}</div></div>;
                  ...
                  if (isP2) return <div key={idx} className="flex justify-end"><div className="max-w-[85%]"><div className="bg-stone-700/80 text-stone-200 px-2 py-1.5 rounded-lg shadow-lg border-r-4 border-orange-500"><div className="text-xs">{formatLogMessage(cleanLog)}</div></div></div></div>;
                })}
                <div ref={logEndRef} />
              </>
            )}
