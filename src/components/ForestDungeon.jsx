...
const ForestDungeon = () => {
  ...
  const BossCard = ({ bossChar, combatBaseOverride: bossCombatBaseOverride }) => {
    ...
    return (
      <CharacterCardContent
        ...
        fallbackOverride={<span className="text-7xl">{levelData?.boss?.icon || '🌲'}</span>}
        showHpBar
  ...
  if (gameState === 'fighting') {
    ...
    const renderLogEntries = (isMobile = false) => (
      combatLog.length === 0 ? (
        <p className="text-stone-500 italic text-center py-6 text-xs">
          {isMobile ? 'Appuyez sur "Lancer"...' : 'Cliquez sur "Lancer le combat" pour commencer...'}
        </p>
      ) : (
        <>
          {combatLog.map((log, idx) => {
            ...
            if (!isP1 && !isP2) {
              if (log.includes('🏆')) return (
                <div key={idx} className="flex justify-center my-3">
                  <div className="bg-stone-100 text-stone-900 px-4 py-2 font-bold text-sm shadow-lg border border-stone-400">{cleanLog}</div>
                </div>
              );
              if (log.includes('💀')) return (
                <div key={idx} className="flex justify-center my-3">
                  <div className="bg-red-900 text-red-200 px-4 py-2 font-bold text-sm shadow-lg border border-red-600">{cleanLog}</div>
                </div>
              );
              if (log.includes('💚')) return (
                <div key={idx} className="flex justify-center my-2">
                  <div className="bg-green-900/50 text-green-300 px-3 py-1 text-xs font-bold border border-green-600">{cleanLog}</div>
                </div>
              );
              if (log.includes('---') || log.includes('⚔️')) return (
                <div key={idx} className="flex justify-center my-2">
                  <div className="bg-stone-700 text-stone-200 px-3 py-1 text-xs font-bold border border-stone-500">{cleanLog}</div>
                </div>
              );
              return (
            ...
            if (isP1) return (
              <div key={idx} className="flex justify-start">
                <div className="max-w-[85%]">
                  <div className="bg-stone-700 text-stone-200 px-2 py-1.5 shadow-lg border-l-4 border-blue-500">
            ...
            if (isP2) return (
              <div key={idx} className="flex justify-end">
                <div className="max-w-[85%]">
                  <div className="bg-stone-700 text-stone-200 px-2 py-1.5 shadow-lg border-r-4 border-purple-500">
                    <div className="text-xs">{formatLogMessage(cleanLog)}</div>
                  </div>
                </div>
              </div>
        ...
        <div className="max-w-[1800px] mx-auto pt-20 md:pt-20">
          ...
          <div className="flex justify-center gap-2 md:gap-4 mb-3 md:mb-4">
            {combatResult === null && (
              ...
              >
                ▶️ Lancer{window.innerWidth < 400 ? '' : ' le combat'}
              </button>
            )}
            <button
          ...
          {combatResult === 'victory' && (
            <div className="flex justify-center mb-3">
              <div className="bg-stone-100 text-stone-900 px-6 py-2 rounded-lg font-bold text-base md:text-xl animate-pulse shadow-2xl border-2 border-stone-400">
          ...
          {combatResult === 'defeat' && (
            <div className="flex justify-center mb-3">
              <div className="bg-red-900 text-red-200 px-6 py-2 rounded-lg font-bold text-base md:text-xl shadow-2xl border-2 border-red-600">
                💀 {player.name} a été vaincu... 💀
              </div>
            </div>
          )}

          {/* ── MOBILE : mini-cartes côte à côte + journal ── */}
          <div className="md:hidden flex flex-col gap-2">
            ...
            {/* Journal de combat mobile */}
            <div className="bg-stone-950/85 border border-stone-700/80 rounded-xl shadow-2xl flex flex-col"
              style={{ height: 'calc(100dvh - 300px)', minHeight: '260px', maxHeight: '480px' }}>
              <div className="bg-stone-900 px-3 py-2 border-b border-stone-700 rounded-t-xl flex-shrink-0">
          ...
          <div className="hidden md:flex gap-4 items-stretch justify-center text-sm md:text-base">
            ...
            <div className="order-2 w-full md:w-[600px] lg:w-[500px] lg:flex-1 lg:min-w-[400px] md:flex-shrink-0 lg:flex-shrink flex flex-col">
              <div className="bg-stone-950/85 border border-stone-700/80 rounded-xl shadow-2xl flex flex-col h-[600px]">
                ...
                <div ref={logContainerRef} className="flex-1 overflow-y-auto p-4 space-y-3 scrollbar-thin scrollbar-thumb-stone-600 scrollbar-track-stone-800">
                  {renderLogEntries(false)}
                </div>
              </div>
            </div>

            <div className="order-3 w-full md:w-[340px] lg:w-auto md:flex-shrink-0">
